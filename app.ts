interface Store {
  currentPage: number;
  // feeds: []; // 빈 배열로 타입을 지정하면 타입을 명시할 수 없다.
  feeds: NewsFeed[];
}
...
interface NewsFeed extends News {
  ...
  read?: boolean; // ?는 해당 property가 있거나 없을 수 있을 때 작성한다.
}
...
// const container = document.getElementById("root");를 아래와 같이 타입을 추가한다.
...
function makeComment(comments: NewsComment[]): string {
  ...
  for (let i = 0; i < comments.length; i++) {
    ...
    commentString.push(`        
      <div class="pl-${comment.level * 40} mt-4">
